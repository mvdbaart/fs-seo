...
- **Google Bedrijfsprofiel (GBP)**: `services/gbpService.js` (accounts, locations, profile health) and `services/gbpPerformanceService.js` (Maps metrics). Authentication is **OAuth, not a service account** — Google's setup docs state the Business Profile API needs an OAuth 2.0 client ID because the data is user-owned, and domain-wide delegation isn't available on a consumer account. The previous service-account implementation silently never connected. Three gotchas: (1) the Performance API ships with **0 QPM** until Google approves the *Application for Basic API Access* form (requires a verified profile active 60+ days) — that's a distinct failure mode from "API disabled", and `classifyGoogleError` separates them; (2) in `fetchMultiDailyMetricsTimeSeries` the query params mix casing — `dailyMetrics`/`dailyRange` are camelCase but `start_date`/`end_date` are snake_case, so the query string is built by hand with `URLSearchParams`; (3) `value` is an int64-as-string and **an absent `value` means 0, not null** — days with no data are omitted from `datedValues` entirely. There is no aggregate impressions metric: profile views are the sum of the four `BUSINESS_IMPRESSIONS_*` values. Both period windows come from **one** request (previous start → current end) and are split in JS.
- **Places API (New)**: `services/placesService.js`, the only source for competitor ratings. Auth is a plain API key (`FS_PLACES_API_KEY` → `FS_SEO_GSC_API` → settings `places_api_key`). Two traps: the field masks are **asymmetric** (Text Search wants `places.rating`, Place Details wants `rating`), and `rating`/`userRatingCount` fall in the **Enterprise** billing tier while Google charges at the most expensive field in the request — so every fetch costs money. Three cache layers guard that, and the middle one is load-bearing: a `hasSnapshot(projectId, 'places', 'rating', today)` check short-circuits to `buildFromSnapshots` with **zero** network calls, capping spend at (1 + competitors) requests/day no matter how often the dashboard loads. `place_id` is resolved via `searchText` exactly once and stored on `competitors.place_id`; a manual refresh bypasses the daily guard but **never** re-runs `searchText`. Competitor matching goes by website host first and **never falls back to the first search result** — an unmatched competitor becomes a visible `dataGap`, because a silent wrong match is worse than a gap. Every outbound call is `console.log`ged so the count stays auditable.
- **Shared OAuth**: `services/googleOAuth.js` holds one client (`google_oauth_client_id`/`_secret`, falling back to the `google_ads_*` pair) with a per-product refresh token, because a refresh token is bound to the scopes granted at consent time. `node server/oauth-setup.js gbp|ads` runs the consent flow and stores the token. Note `prompt: 'consent'` in the auth URL is required — without it a second authorization returns no refresh token and the script appears to succeed while storing nothing.
...
- `services/insightsEngine.js` — data only, no prose. `buildWindows(days, lagDays=3)` yields a current and previous window (GSC lags 2–3 days, GA4 ~1, so one shared window keeps sources comparable). Seven collectors (GSC, GA4, GBP, Places, rankings, PageSpeed, crawl) run under `Promise.allSettled`, so one broken source degrades to an honest `{connected:false, message}` instead of failing the request. **Adding a collector means four order-coupled edits in `buildInsightsReport`**: the `allSettled` array, the destructuring, `COLLECTOR_NAMES` (index-aligned), and the `collectors` object. `movers` is a flat merge across collectors, so mover keys must be globally unique. `persistSnapshots(projectId, collectors)` takes an object, not positional args — with seven collectors a shifted argument is a silent bug. Results cache 10 minutes per `projectId:days`; without that cache every tab switch fires ~10 Google calls. **Do not mount this on `GET /api/projects/:id/dashboard`** — that route must stay fast.
...
`lowerIsBetter: true` applies to `gsc.position`, `ga4.bounceRate`, `pagespeed.lcp/cls`, `crawl.errors`, `rankings.avgPosition`. An average position moving 8.0 → 5.0 is `direction:'down'` **and** `sentiment:'positive'`. Any collector that computes its own sentiment is a bug. A `THRESHOLDS` table (`minAbs`/`minPct`/`minBase`) suppresses noise — `minBase` is what stops 2 → 4 clicks reading as "+100%". A signal id missing from `THRESHOLDS` gets `magnitude: 0` and sorts last in every highlight list, so **always register new ids there**. When `previous === 0`, `deltaPct` is `null`, so every template using it needs a null branch or it prints `Infinity%`.

Two narrator details that bite: `describe()`'s fallback formats with `fmtCount`, which rounds — a 4.6 rating would render as "5", so every rating signal needs an explicit template using `fmtRating`. And `SOURCE_LABELS` exists in **two** files (`insightsNarrator.js` with an article for mid-sentence use, `DashboardView.jsx` short for badges); `sourceTitle()` strips the article for headings.

`places.ratingGap` only carries a `previous` when the set of matched competitor place ids is unchanged (stored in the snapshot's `meta`). Adding a higher-rated competitor would otherwise shrink the gap and be narrated as "your lead shrank", which is factually wrong.
